...
4. `references/radix-testing-patterns.ts` — if component wraps a Radix primitive (check for `@radix-ui` imports in source)
...
When generating tests for components that wrap Radix UI primitives, read `references/radix-testing-patterns.ts` for known jsdom quirks and required patterns:

...
| File | Purpose |
|------|---------|
| `references/spec-type.ts` | `ComponentSpec` type definition |
| `references/categories.ts` | Valid categories and placement rules |
| `references/animation-map.ts` | Animation patterns, presets, and core functions |
| `references/keyboard-map.ts` | Keyboard pattern → keys/behavior |
| `references/a11y-contract.ts` | Class → ARIA requirements |
| `references/form-patterns.ts` | Form integration patterns |
| `references/default-conventions.ts` | Token/size/variant defaults |
| `references/engine-api.ts` | Available engine/animation imports |
| `references/radix-primitives.ts` | Available Radix primitives |
| `references/tokens-semantic.ts` | Available semantic tokens |
...
| animations | Trigger-sequence pairs: `{ trigger, sequence, vars?, delegate?, onComplete?, deps?, direction? }[]`. Extract from useAnimations defaults, event handlers, effects. See `references/animation-map.ts` for patterns per componentClass. `deps` for value-reactive triggers, `onComplete` for post-animation callbacks. |
...
Use `references/animation-map.ts` to fill animation defaults based on `componentClass`.
...
Use `references/a11y-contract.ts` to fill accessibility defaults.

Use `references/default-conventions.ts` to fill token value defaults.
...
1. **Read `references/tokens-semantic.ts`** — verify every `var(--move-*)` semantic token exists in the exported constants.
2. **Read `references/tokens-primitive.ts`** — if referencing a primitive token (e.g. `--move-space-*`, `--move-radius-*`), verify it exists in the exported constants (`SPACING`, `RADII`, `TYPOGRAPHY`, etc.).
...
Note: Specs use inline `as const` assertions instead of importing from `references/spec-type.ts`. The spec-type file is a reference for skills, not a runtime dependency.
...
4. **All token values must reference semantic tokens** — `var(--move-*)` from `references/tokens-semantic.ts`

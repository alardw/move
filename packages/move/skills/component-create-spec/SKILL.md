...
| animationCapabilities | Tier-2 animation BEYOND the declarative `animations` system — declare it so imperative animation is never ad-hoc (enforced by `check:animation-capabilities`). `'slidingIndicator'` = track an active element via the `usePositionTracker` hook (Tabs/Pagination/ToggleGroup/TableOfContents). `'valueLoop'` = loop a JS value/proxy via raw anime.js + a render callback (Loader/Skeleton). `'measureThenAnimate'` = enter values that depend on a post-render measurement, via imperative `moveAnimate` (ChatBubble). `'scrollApi'` = imperative scroll/gesture, no anime.js (Carousel). **Omit entirely when the component animates only through `animations`** — which is the norm. Prefer `slidingIndicator` (the shared hook) over an `animatePosition` trigger for any active-element indicator. |

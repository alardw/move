...
11. **Always use FormField** — every form input (InputText, Select, Checkbox, etc.) must be wrapped in `FormField.Root` with `FormField.Label` and `FormField.Field`. Never render a bare input without its FormField wrapper. **Exception:** a standalone toolbar/search input (a search box that is not part of a form) may omit FormField, but MUST carry an accessible name via `aria-label`.

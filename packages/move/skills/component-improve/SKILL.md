...
Amend an existing component spec based on a natural language change request. Only reviews NEW defaults — existing defaults are preserved.
...
For each NEW defaultable prop, propose a default using this format:
- `prop`: name
- `proposed`: value
- `why`: one sentence
- `confidence`: high/medium/low

**Critical**: Do NOT re-review existing defaults. Only new props get reviewed.
...
2. **Preserve existing defaults** — never re-review or modify existing defaults unless the user's change request explicitly targets them

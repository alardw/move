...
export const spec = {
  ...
  compound: false,
  rootElement: 'div',
  slots: [{ name: 'root', element: 'div', type: 'none', description: 'Flex container element' }],
  ...
  props: [
    ...
    {
      name: 'justify',
      type: "'start' | 'center' | 'end' | 'between' | 'evenly'",
      default: "'start'",
      moveSpecific: true,
    ...
    },
    {
      ...
      type: 'string',
      moveSpecific: true,
      description: 'Container width threshold (px) below which direction collapses to column',
    },
    {
      name: 'flex',
      type: "1 | 'auto' | 'none'",
      moveSpecific: true,
      description:
        "Flex sizing along the parent's main axis. 1 = grow to fill remaining space (with min-width:0 so wide children scroll instead of overflowing the row); 'auto' = size to content but allow grow/shrink; 'none' = fixed at content size.",
    },
    {
      name: 'fill',
    ...
    },
    {
      ...
      moveSpecific: false,
      description: 'Child elements to lay out',
    },
  ],
  ...
  tokens: [
    ...
    {
      name: '--move-stack-gap-xs',
      value: 'var(--move-spacing-xs)',
      description: 'Gap when gap=xs',
    },
    {
      name: '--move-stack-gap-sm',
      value: 'var(--move-spacing-sm)',
      description: 'Gap when gap=sm',
    },
    {
      name: '--move-stack-gap-md',
      value: 'var(--move-spacing-md)',
      description: 'Gap when gap=md',
    },
    {
      name: '--move-stack-gap-lg',
      value: 'var(--move-spacing-lg)',
      description: 'Gap when gap=lg',
    },
    {
      name: '--move-stack-gap-xl',
      value: 'var(--move-spacing-xl)',
      description: 'Gap when gap=xl',
    },
  ],
  ...
  renderContracts: [
    {
      id: 'direction-data-attr',
      description:
        'Direction is applied via data-direction attribute, flex-direction resolved in CSS',
    },
    {
      id: 'gap-data-attr',
      description:
        'Gap is applied via data-gap attribute, gap value resolved via component tokens in CSS',
    },
    {
      id: 'align-data-attr',
      description: 'Align is applied via data-align attribute, align-items resolved in CSS',
    },
    {
      id: 'justify-data-attr',
      description: 'Justify is applied via data-justify attribute, justify-content resolved in CSS',
    },
    {
      id: 'wrap-data-attr',
      description: 'Wrap is applied via data-wrap boolean attribute when wrap=true',
    },
    {
      id: 'collapse-responsive',
      description:
        'When collapseBelow is set, a ResizeObserver sets data-collapsed on the root when container width is below the threshold, overriding direction to column',
    },
    {
      id: 'stagger-opt-in',
      description:

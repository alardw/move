...
export const spec = {
  ...
  rootElement: 'span',
  slots: [
    { name: 'root', element: 'span', description: 'Radix Slider.Root element with track and thumbs' },
    { name: 'track', element: 'span', description: 'Radix Slider.Track — the full slider rail' },
    { name: 'range', element: 'span', description: 'Radix Slider.Range — filled portion of the track' },
    { name: 'thumb', element: 'span', description: 'Radix Slider.Thumb — draggable handle(s)' },
    { name: 'value', element: 'span', description: 'Optional value display label(s) alongside slider' },
  ],

  subComponents: [],

  props: [
    { name: 'min', type: 'number', default: '0', moveSpecific: true, description: 'Minimum slider value' },
    { name: 'max', type: 'number', default: '100', moveSpecific: true, description: 'Maximum slider value' },
    { name: 'step', type: 'number', default: '1', moveSpecific: true, description: 'Step increment' },
  ...
  renderContracts: [
    { id: 'hook-normalizes-value', description: 'useInputRange hook normalizes single number to number[] internally; onValueChange always returns number[]' },
    { id: 'show-value-wrapper', description: 'When showValue=true, slider is wrapped in a div.wrapper with flex layout and value label span(s)' },
    { id: 'range-dual-value-labels', description: 'When value is array (range mode) and showValue=true, two value labels are rendered (start and end)' },
    { id: 'single-value-label', description: 'When single value and showValue=true, only one value label is rendered (end)' },
    { id: 'format-value-default', description: 'Default formatValue is String(v); custom formatValue overrides display' },
    { id: 'animate-on-first-thumb', description: 'Interaction animation ref is applied to the first thumb only' },
    { id: 'handlers-on-all-thumbs', description: 'Interaction animation handlers (hover/press) spread to all thumbs' },
    { id: 'vertical-orientation', description: 'Vertical orientation rotates layout; wrapper uses flex-column; track uses width instead of height' },
    { id: 'className-on-wrapper-when-showValue', description: 'When showValue=true, className/style go on the wrapper div, not the Slider.Root' },
    { id: 'ref-forwards-to-slider-root', description: 'Component ref forwards to the Radix Slider.Root span element' },
  ],

  tokens: [
    { name: '--move-range-track-height', value: '6px', description: 'Track rail height (or width in vertical)' },
    { name: '--move-range-track-bg', value: 'var(--move-bg-muted)', description: 'Track background color' },
    { name: '--move-range-track-radius', value: 'var(--move-rounded-full)', description: 'Track border radius' },
    { name: '--move-range-range-bg', value: 'var(--move-primary)', description: 'Filled range background color' },
    { name: '--move-range-thumb-size', value: '20px', description: 'Thumb width and height' },
    { name: '--move-range-thumb-bg', value: 'var(--move-primary-fg)', description: 'Thumb background color' },
    { name: '--move-range-thumb-border', value: '2px solid var(--move-primary)', description: 'Thumb border' },
    { name: '--move-range-thumb-shadow', value: 'var(--move-shadow-subtle)', description: 'Thumb box shadow' },
    { name: '--move-range-thumb-focus-ring', value: '0 0 0 var(--move-focus-ring-width) var(--move-focus-ring-color)', description: 'Thumb focus ring box-shadow' },
    { name: '--move-range-value-color', value: 'var(--move-fg-muted)', description: 'Value label text color' },
    { name: '--move-range-value-size', value: 'var(--move-font-size-sm)', description: 'Value label font size' },
    { name: '--move-range-value-min-width', value: '2ch', description: 'Value label minimum width' },
  ],

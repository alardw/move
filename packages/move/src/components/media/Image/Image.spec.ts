...
export const spec = {
  ...
  props: [
    ...
    { name: 'sources', type: 'ImageSource[]', moveSpecific: true, description: 'Array of responsive image variants with intrinsic pixel widths for container-based resolution' },
  ...
  variants: {
    ...
    position: ['center', 'top', 'bottom', 'left', 'right', 'top left', 'top right', 'bottom left', 'bottom right'] as string[],
  },
  sizes: [] as string[],
  ...
  renderContracts: [
    { id: 'responsive-sources', description: 'When sources array is provided, uses ResizeObserver on root to select the best src based on container width * devicePixelRatio' },
    { id: 'fallback-on-error', description: 'Shows fallback slot when img fails and no fallbackSrc is set; switches to fallbackSrc when available' },
    { id: 'error-reset-on-src', description: 'Resets error state when src or resolved source changes' },
    { id: 'action-conditional', description: 'Action overlay only renders when action prop is provided and image is not in fallback state' },
  ...
  testing: {
    ...
    aria: [
      'img element has alt attribute for accessibility',
    ],
  },

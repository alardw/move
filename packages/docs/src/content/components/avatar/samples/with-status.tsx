    ...
    <Stack gap="md">
      {people.map((p) => (
        <Stack key={p.name} direction="row" gap="md" align="center">
          {/* composite-purity-ignore: Avatar has no built-in status dot — a relative wrapper + absolutely positioned coloured dot; no Move primitive expresses this overlay */}
          <span style={{ position: 'relative', display: 'inline-flex' }}>
            <Avatar.Root>
              <Avatar.Image src={p.src} alt={p.name} />
              <Avatar.Fallback>{p.initials}</Avatar.Fallback>
            </Avatar.Root>
            {/* composite-purity-ignore: status dot — absolutely positioned coloured circle, no Move primitive */}
            <span
              aria-hidden
              // composite-purity-ignore: inline positioning for the status dot; no Move primitive
              style={{
                position: 'absolute',
                right: -2,
                bottom: -2,

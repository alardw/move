...
const descriptions: Record<(typeof variants)[number], string> = {
  ...
  ghost:
    'No chrome at all — just the monospace font. For when surrounding context already separates the snippet.',
};
    ...
    <Stack gap="lg">
      {variants.map((v) => (
        <Stack key={v} gap="xs">
          ...
          <Text>
            Inline <Code variant={v}>useAnimations(config, refs)</Code> renders with the {v}{' '}
            treatment.
          </Text>
        </Stack>
      ))}

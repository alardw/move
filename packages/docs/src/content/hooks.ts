...
export const HOOKS_REGISTRY: HookDoc[] = [
  ...
  {
    name: 'useInView',
    signature: 'useInView<T>({ rootMargin?, threshold?, once? }): { ref, inView }',
    summary:
...
<div ref={ref} tabIndex={isOverflowing ? 0 : undefined}>…</div>`,
  ...
  {
    name: 'useAnimations',
    signature: 'useAnimations(config, refs, states?, options?): { handlers, runExit, … }',
    summary:
  ...
  // ── Theming & surfaces ──────────────────────────────────────────
  {
    ...
    summary: 'Read the active theme and switch it from anywhere inside a MoveRoot.',
  ...
  },
  {
    ...
    summary: 'Read the current stacking layer depth for correct z-index in nested overlays.',
    category: 'Theming & surfaces',
  },
  ...
  },
  {
    ...
    signature: 'useResolvedIcon(name: string, size: number): React.ReactNode | null',
    summary:

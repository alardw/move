...
const barrelSrc = readFileSync(resolve(here, '../../move/src/index.ts'), 'utf8');
...
const GENERAL_LAYER_PREFIXES = ['./engine', './hooks', './animation', './infrastructure'];
...
const generalHooks = [...barrelHooks.entries()]
  .filter(
    ([name, spec]) =>
      EXTRA_GENERAL.has(name) || GENERAL_LAYER_PREFIXES.some((p) => spec.startsWith(p)),
  )
  .map(([name]) => name);
...
describe('hooks registry', () => {
  ...
  it('has no stale entries (every registry hook is still a move export)', () => {
    ...
    expect(stale, `these registry entries are no longer exported from 'move'`).toEqual([]);
  });

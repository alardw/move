...
const HOLDS: HighlightItem[] = [
  {
    ...
    text: (
      <>
        <Code>fromPattern</Code> — the design pattern this resolves, e.g.{' '}
        <Code>'item-gallery'</Code>.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <Code>adapter</Code> — where its data comes from. Only a root collection has one; a child
        tile receives its item as a prop.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <Code>decisions</Code> — every axis of the pattern, resolved to a value. The whole set,
        explicit — no overrides, no deltas.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <Code>children</Code> — each delegated slot mapped to the child composite that fills it,
        e.g. <Code>{`{ item: 'ApodCard' }`}</Code>.
      </>
    ),
  },
  {
    ...
    text: (
      <>
        <Code>labels</Code> — the user-facing strings this instance renders, for i18n.
      </>
    ),
  },
];
        ...
        >
          <Stack gap="md">
            ...
            <Text color="muted" size="sm">
              This page is still being written. The <Code>CompositeSpec</Code> shape is settling as
              we build real composites — an earlier layout-tree idea that used to live here has been
              replaced by the pattern-plus-decisions model above. The current type is{' '}
              <Code>CompositeSpec</Code> (exported from <Code>move</Code>); you can see it resolved
              in the nasa-explorer example’s <Code>ApodGallery</Code> and <Code>ApodCard</Code>{' '}
              specs. A full walkthrough — every field, with a real spec shown inline — lands once
              the shape is locked.
            </Text>
          </Stack>
        </Section>

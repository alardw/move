...
import { Stack, Align, Heading, Text, Breadcrumb, Code, Badge, Icon, Table, Alert } from 'move';
...
const WIDTH_KINDS = [
  ...
  },
  {
    ...
    resolved: 'The container, which fits as many as it can and shares out the rest.',
  },
];
      ...
      <Stack gap="xl" flex={1}>
        ...
        <Stack gap="sm">
          ...
          <Text color="muted" size="lg">
            Sizing in Move works from the outside in. Each container decides how much room its
            children get, and each child fits the space it is handed. That is what lets a panel fill
            the screen and scroll on its own when its content runs long.
          </Text>
        ...
        >
          ...
          <Text size="sm" color="muted">
            All three keep the last word with the container, which is what lets one tree serve a
            phone and a wide monitor. A preference travels as a floor the container may raise —{' '}
            <Code>minChildWidth</Code> caps itself at the room available, so a child that asks for
            more than it can have wraps onto its own row.
          </Text>
          ...
          <Stack gap="sm">
            ...
            <Text size="sm" color="muted">
              ...
              <RouterLink to="/components/splitter">Splitter</RouterLink> panel starts at a size the
              reader then drags wherever they like. An{' '}
              <RouterLink to="/components/image">Image</RouterLink> or{' '}
              <RouterLink to="/components/video-player">VideoPlayer</RouterLink> carries the
              dimensions of the media itself. A{' '}
              <RouterLink to="/components/skeleton">Skeleton</RouterLink> takes the shape of the
              content it stands in for. In each of those the number is the point. Everywhere else a
              size comes from a scale, and{' '}
              <RouterLink to="/systems/forms">form controls</RouterLink> have one of their own.
            </Text>
          </Stack>
        </Section>
        ...
        >
          ...
          <Text size="sm" color="muted">
            No layout component knows what a viewport is. That absolute enters once, via{' '}
            <Code>{'<MoveRoot fullHeight>'}</Code>, and everything below is relative to it — so the
            same tree works inside a 360px card as it does filling a window.
          </Text>
        </Section>
        ...
        >
          <Table>
            ...
            <Table.Body>
              <Table.Row>
                ...
                <Table.Cell>
                  <Text size="sm">
                    The parent is a flex container — a Stack, Align, or Splitter.Root. Correct
                    whether or not you have siblings, so prefer it.
                  </Text>
                </Table.Cell>
              </Table.Row>
              <Table.Row>
                ...
                <Table.Cell>
                  <Text size="sm">
                    ...
                    <Code>Splitter.Panel</Code> or a Card body. Only correct as the only child: with
                    siblings it claims the full height and overflows.
                  </Text>
                </Table.Cell>
              </Table.Row>
            </Table.Body>
          ...
          <Alert variant="info" title="Why both minimums, for one axis?">
            A flex item's main axis belongs to its parent, and a child can't read its parent's
            direction in CSS. So <Code>remaining</Code> sets <Code>min-width: 0</Code> and{' '}
            <Code>min-height: 0</Code> together; each is a no-op on the axis that isn't the main
            one. That also means you never pass an orientation — <Code>remaining</Code> is correct
            in a row and a column alike.
          </Alert>
        </Section>
        ...
        >
          ...
          <Text size="sm" color="muted">
            Drag the divider. Each pane scrolls on its own, the frame keeps its size, and the page
            behind it stays where it is. The value switches from <Code>remaining</Code> to{' '}
            <Code>parent</Code> exactly where the parent stops being a flex container — inside{' '}
        ...
        >
          <Stack gap="md">
            <Stack gap="xs">
              ...
              <Text size="sm" color="muted">
                It grows, but it stays floored at its content height, so it hands a content-shaped
                height to everything below and the scroll region never becomes a scrollport. Use{' '}
                <Code>fill="remaining"</Code>.
              </Text>
            </Stack>
            <Stack gap="xs">
              ...
              <Text size="sm" color="muted">
                <Code>height: 100%</Code> ignores the siblings above it, so the box is as tall as
                the whole parent and overflows by however much they took.
              </Text>
            </Stack>
            <Stack gap="xs">
              <Text weight="medium">
              ...
              <Text size="sm" color="muted">
                Hidden stays a scroll container: focus a field below the fold and the browser
                scrolls the box, with no scrollbar to get back. <Code>clip</Code> can't — which is
                why Stack's prop is <Code>clip</Code> and there is no <Code>hidden</Code>.
              </Text>
            </Stack>
            <Stack gap="xs">
              <Text weight="medium">An unsized mount node</Text>
              <Text size="sm" color="muted">
                <Code>fullHeight</Code> sizes <Code>html</Code> and <Code>body</Code>, but your
                mount node — usually <Code>#root</Code> — sits between them and the tree. If it
                isn't <Code>height: 100%</Code>, the chain breaks at the first link. A dev-only
                warning fires when it does.
              </Text>
            </Stack>
          </Stack>
          ...
          <Align>
            <Text size="sm" color="muted">
              Props live on <RouterLink to="/components/stack">Stack</RouterLink>,{' '}
              <RouterLink to="/components/align">Align</RouterLink>,{' '}
              <RouterLink to="/components/scroll-area">ScrollArea</RouterLink> and{' '}
              <RouterLink to="/components/splitter">Splitter</RouterLink>.
            </Text>
          </Align>
        </Section>
      </Stack>

...
import { HOOKS_REGISTRY, HOOK_CATEGORY_ORDER, type HookCategory } from '../../content/hooks';
      ...
      <Stack gap="xl" flex={1}>
        ...
        <Stack gap="sm">
          ...
          <Text color="muted" size="lg">
            A small set of React hooks Move ships for the cases that need to cross component lines.
            Same shape as React&apos;s built-ins, no extra ceremony.
          </Text>
            ...
            <Stack gap="lg">
              {HOOKS_REGISTRY.filter((h) => h.category === cat).map((hook) => (
                <Stack key={hook.name} gap="xs">
                  ...
                  {hook.example && <CodeBlock code={hook.example} language="tsx" />}
                </Stack>
              ))}
            </Stack>
          </Section>

...
import { VALIDATION, groupsForEntity, tallyFor, type EntityDef, type Status } from './validation-spec';

const TOC: TocItem[] = [
  { href: '#coverage', label: 'Overview' },
  { href: '#ambient', label: 'Ambient tooling' },
  ...VALIDATION.entities.map((e) => ({ href: `#${e.key}`, label: e.title })),
];

const TOTAL = VALIDATION.entities.reduce(
  (acc, e) => {
    const t = tallyFor(e);
    return { check: acc.check + t.check, gap: acc.gap + t.gap };
  },
  { check: 0, gap: 0 },
);
...
function StatusCell({ status, check }: { status: Status; check?: string }) {
  ...
  return <Badge variant="soft" color="orange">gap</Badge>;
}
  ...
  return (
    <Section id={entity.key} title={entity.title} lede={entity.blurb}>
      <Stack direction="row" gap="xs" wrap>
        <Badge variant="soft" color="green">{t.check} enforced</Badge>
        <Badge variant="soft" color="orange">{t.gap} gaps</Badge>
      </Stack>
      <Table>
        <Table.Header>
          <Table.Row>
          ...
          <Table.Group key={group.id} collapsible={false}>
            ...
            {rules.map((r, i) => (
              <Table.Row key={r.id}>
                <Table.Cell>
                  <Stack gap="xs">
                    <Text size="sm"><Code>{i + 1}</Code> {r.rule}</Text>
                    <Text size="xs" color="muted">{r.why}</Text>
                  </Stack>
                </Table.Cell>
                <Table.Cell><StatusCell status={r.status} check={r.check} /></Table.Cell>
              </Table.Row>
            ))}
          </Table.Group>
        ))}
      ...
      <Stack gap="xl" flex={1}>
        ...
        <Stack gap="sm">
          ...
          <Text color="muted" size="lg">
            ...
            Which rules apply to each entity is computed from the entity's traits, not assigned by hand.
          </Text>
          <Stack direction="row" gap="xs" wrap>
            <Badge variant="soft" color="green">{TOTAL.check} enforced</Badge>
            <Badge variant="soft" color="orange">{TOTAL.gap} gaps</Badge>
          </Stack>
          <Text size="sm" color="muted">
            <Code>check</Code> = a deterministic gate enforces it. <Code>gap</Code> = mechanizable, but
            no check exists yet — never a human call. Beneath all of it, the ambient tooling below runs
            ...
            <Table.Body>
              {VALIDATION.ambient.map((t) => (
                <Table.Row key={t.tool}>
                  <Table.Cell><Code>{t.tool}</Code></Table.Cell>

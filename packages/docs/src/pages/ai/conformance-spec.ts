...
const RULES: RuleDef[] = [
  ...
  { id: 'styles-23', group: 'styles', rule: 'Spec token values match the CSS', why: 'The spec declares each token’s value; if the CSS differs, the documented token table is wrong.', enforcement: C('check', 'spec-tokens') },
  ...
  { id: 'animation-4', group: 'animation', rule: 'Motion targets markers, and hands back', why: 'Animations select data-* markers so a stylesheet can see which properties are spoken for. CSS may set state 1 under an attribute the component removes once the animation takes over (Chart\u2019s [data-enter="pending"]) — one direction only. A finished animation should leave no inline style behind, so state 2 applies on its own; anything that snaps back on cleanup was using an animation to hold a state. Handing back is what an ENTER does, because an enter has a state 2 — the class for where it arrived. An exit does not: what holds an element once it has gone belongs to the component, not the animation, so an exit keeps what it wrote.', enforcement: { component: { status: 'gap' } } },
  { id: 'animation-5', group: 'animation', rule: 'A stagger targets an element we own', why: 'A stagger queries its target for children, so the target must be an element the component renders. A third-party component owns its own ref, styling and reparenting, and an animated ref put there competes with the library for the same node — Dropdown staggered a div in its menu and the Radix element in its sub-menu, and only the first one animated.', enforcement: C('check', 'stagger-container') },
  { id: 'animation-6', group: 'animation', rule: 'A reveal that fires on page load is opt-in', why: 'A component cannot see the others on the page. One table settling its rows is a nice touch; a table, a timeline and a paginator all settling at once is the page shaking itself out, and each component made a locally correct decision to get there. So a stagger on a mount/enter trigger sits behind a `stagger` prop, default off, and the caller — who can see the whole page — opts in. This is NOT a rule against motion: a stagger that answers something the reader just did (a popover opening, files landing after a drop, a page change sliding) fires alone, is the feedback for that action, and stays on. The test is what triggers it, not what it looks like.', enforcement: { component: { status: 'gap' } } },
  { id: 'styles-24', group: 'styles', rule: 'Truncation comes from the utility, not from a slot', why: 'A local `text-overflow: ellipsis` is a component deciding for itself how text gets cut, and it gives up three things the [data-truncate] utility supplies: `middle`, which keeps the end of the string — a filename truncated at the end loses its extension, which is the part a reader scanning a list is looking for; the tooltip that hands the full string back on hover; and `overflow: clip visible` rather than `hidden`, which clips both axes to the LINE box and takes the tails off every p, g and y. FileUpload had all three from six lines of CSS.', enforcement: C('check', 'hand-rolled-truncation') },
  { id: 'behavior-5', group: 'behavior', rule: 'A component composed onto another’s element leaves it its data-state', why: 'asChild merges two components onto one DOM node. Classes concatenate and handlers compose, so sharing those is harmless — but data-state holds one value, and Radix standardises on that key across every primitive, so any two composed this way collide. Wrapping a ToggleGroup item in a Tooltip put closed on the button, which matches neither on nor off, so all eight of ToggleGroup’s state rules fell through and the selected segment drew as if nothing were chosen — silently, with aria-checked correct throughout. There is a host and a guest: the host renders the element and its CSS is written against it, so it keeps the plain key; the guest writes data-move-{owner}-state, which also means two guests on one node do not collide. Judged per element, not per file, because a trigger is a guest under asChild and a host without it.', enforcement: C('check', 'guest-state') },
  { id: 'surface-1', group: 'surface', rule: 'A ground is the attribute and the provider, or neither', why: 'data-surface remaps the relative tokens for an element AND its descendants; SurfaceProvider tells React the same thing. Setting one without the other splits the halves: the stylesheet calls the panel subtle while anything inside asking what it landed on still hears the ground OUTSIDE, so it steps to the wrong shade and can land on the colour of the panel it is sitting on. Select carries a comment about exactly that happening, and Alert shipped the same defect — it set the attribute, provided nothing, and declared neither, which is what made it invisible to check:capabilities.', enforcement: C('check', 'surface-contract') },
  { id: 'surface-2', group: 'surface', rule: 'A component that alternates declares owns-surface', why: 'The capability is what check:capabilities verifies both halves through, and it is implied by a slot of kind surface. A component that computes its tone with useSurfaceFlip but declares neither keeps the contract only by accident, with nothing watching. Pinning a tone is different and does not require the declaration — the capability requires useSurfaceFlip, and a panel that deliberately fixes its ground cannot call it.', enforcement: C('check', 'surface-contract') },
  ...
  { id: 'spec-11', group: 'spec', rule: 'A family\u2019s members keep its contract', why: 'A family is a named bundle of promises, joined by components that are the same kind of thing \u2014 which is what separates it from a capability, one promise shared by unrelated components. Dialog and Drawer agree on keyboard, focus, controlled, dismiss, ARIA and capabilities; Accordion and Calendar agree on roving focus and nothing else, which is why they are not a family \u2014 one shared trait is the shape of a capability, not of a family, whether or not it is worth writing one. Defined as data in src/families.ts and read by one check, replacing three scripts that each hardcoded their own membership predicate. Families compose, so a shared core is written once: the old families.state axis restated the controlled field in 71 specs and 11 copies had drifted from it.', enforcement: C('check', 'families') },
  { id: 'spec-10', group: 'spec', rule: 'A component declares the capabilities it has, and satisfies them', why: 'A capability is one contract written once, targeting slot KINDS rather than names, so it covers every component whatever each calls its parts \u2014 the interactive element is `root` in Checkbox, `input` in InputText and `trigger` in Select. Checked both ways: a declared capability whose contract does not hold fails, and so does source that exhibits one without declaring it. The second direction is the one that earns it \u2014 sixteen scroll containers with no focus indicator, and two components setting data-surface with no provider, were all things nobody had written down, which a declaration-only check would never have mentioned.', enforcement: C('check', 'capabilities') },
  { id: 'spec-9', group: 'spec', rule: 'The spec\u2019s animations array describes the animations that ship', why: 'check:spec-drift compares props, defaults, slots, tokens and engine imports and never reads `animations`, so that one field could say anything and stay green. It did: a Sidebar whose staggers were absent from the array entirely, three menus declaring a `[role="menuitem"]` selector months after moving off it, and an `Item.hover` shipped by Dropdown, Select and Autocomplete that no spec mentioned. Compared in both directions \u2014 a trigger the spec claims must exist in the component, and a trigger in a module-scope defaults array must be declared. Module scope is what separates a shipped default from internal wiring, since Select and Autocomplete remap their public open/closed onto Content.enter inside the component.', enforcement: C('check', 'animation-spec-drift') },
  { id: 'styles-18', group: 'styles', rule: 'A component that positions something out of sight contains it', why: '`position: absolute` with no positioned ancestor resolves against the DOCUMENT, so the element escapes every scroll container between it and the page and lands at its static offset from the top of the document \u2014 the scroll height then grows to reach it. Three components shipped that, each a visually-hidden copy: AnimatedText\u2019s accessible full text, Chart\u2019s hidden data table, PasswordStrength\u2019s sr-only label. The components gallery became 17,365px tall against a 907px viewport, a whole screen of empty page past the app shell. body.scrollHeight stayed at the viewport throughout, because nothing in the BODY was tall.', enforcement: C('check', 'containing-block') },
  { id: 'styles-21', group: 'styles', rule: 'The stacking scale has one definition, and the tokens are checked against it', why: '`src/shared/z-layers.ts` names the layers and is exported as public API; the `--move-z-*` tokens are what the browser obeys. Nothing declared that those were the same scale, so they parted: the registry carried invented values (1020\u20131080) nothing rendered at, disagreed with the tokens about whether toast or tooltip sits on top \u2014 so the docs published the opposite of what shipped \u2014 listed two layers that had no token at all, and omitted `dropdown` and `modal`, which components reference twelve times between them. Because it is public, a consumer reading `Z_LAYERS.popover.value` got 1060, above everything Move draws, and would have covered their own dialogs with no way to find out why.', enforcement: C('check', 'z-layers') },
  { id: 'styles-22', group: 'styles', rule: 'Every z-index is a token, and the token\u2019s name says which layer it is in', why: 'Two different jobs wear the same property. LOCAL stacking \u2014 ToggleGroup lifting the active segment over its neighbours\u2019 borders, a player\u2019s controls over the scrim \u2014 is small integers meaningful only against each other, and a layer token would flatten them all to one value and destroy the ordering they exist for. LAYERED stacking competes with components that do not know about it, and the only way to order those is a shared scale. The old rule said components never write a raw z-index, which was false twenty-three times over on the day it was written, so it taught everyone the header could be skipped. A sticky or fixed element is the mechanically detectable case: it leaves its parent\u2019s flow to sit over whatever scrolls past, which is other components by definition \u2014 Table\u2019s sticky header sat at a bare z-index: 1 and lost to anything nearby that lifted itself, while the docs claimed it occupied the sticky layer.', enforcement: C('check', 'z-layers') },
  { id: 'styles-19', group: 'styles', rule: 'A press state sits on the thing being pressed', why: '`:active` matches an ANCESTOR whenever a descendant is pressed, so a transform under it on a container scales everything inside: clicking one segment made a whole toggle group shrink. The sibling controls were safe because their root IS the button, with no descendant to propagate from \u2014 which is what `kind` records, so the rule is checkable rather than a matter of noticing. Only transforms: a container may legitimately tint on :active, because a tint does not move its children.', enforcement: C('check', 'press-scope') },
  { id: 'styles-20', group: 'styles', rule: 'Text rendered as a control\u2019s name belongs to a declared slot', why: 'The declaration gap the other checks cannot see. An unclassed span is not a slot, so it carries no typography role, so check:type-scale has nothing to compare \u2014 and has-label only fires where a label slot already exists, protecting components that already got it right. Two controls shipped labels at body copy this way while every other control rendered at the ui step; nothing was malformed, nothing was declared, so nothing looked.', enforcement: C('check', 'rendered-label') },
  { id: 'styles-17', group: 'styles', rule: 'A list you choose from looks the same wherever it is', why: 'Select, Dropdown, Autocomplete and the media players\u2019 settings menu all render a row you highlight with the keyboard and select with a click, and they matched only by coincidence \u2014 each carried its own *-item-radius, *-item-padding and *-item-bg-highlight, every one resolving to the same value with nothing holding them together. The one written last matched none of them: square rows bleeding into the panel\u2019s rounded corner, a highlight three times Select\u2019s strength, and a selected row that did not darken when focused. Keyed on data-highlighted, because being highlightable is what makes a row an option \u2014 a checkbox is checked and a list row can be active, and neither is walked with arrow keys. A CSS check rather than a capability, because the component this was written for has no spec: ten shared internals sit outside the spec system, which is exactly where the defects were.', enforcement: C('check', 'option-rows') },
  { id: 'styles-16', group: 'styles', rule: 'Every font-size comes from the type scale', why: 'A size that does not resolve into --move-text-* / --move-size-* is off the scale by definition, and will not follow a consumer\u2019s typography. The literal case is visible in review; the one that actually shipped is subtler \u2014 a `*-font-size` token set to `inherit` reads as "follow the context" and behaves that way on a div, but a replaced element (input, textarea, select) does not inherit type from the document and takes the UA default instead, 13.333px in Chrome. Five form fields shipped that as their md default while their sm and lg variants named a step correctly: the only size nobody wrote down was the one that left the system. `inherit` stays legal on a plain declaration, and an em relative to running text is exempt with a reason.', enforcement: C('check', 'type-scale') },
  { id: 'styles-14', group: 'styles', rule: 'A container of focusable children leaves room for their focus rings', why: 'An outset ring is drawn OUTSIDE the element box, reaching --move-focus-ring-room beyond it. A gapped container whose gap is smaller puts each row’s ring on top of its neighbour — worst against a filled row, where the two read as joined. The defect exists only in the RELATIONSHIP between two rules that each look correct alone, which is why it keeps recurring and why the fix belongs on the container (gap: max(<gap>, var(--move-focus-ring-room))) rather than on the ring. Where rows must sit tight, inset the ring instead, per the row/cell/list-item convention.', enforcement: C('check', 'focus-ring-room') },
  ...
  { id: 'exports-6', group: 'exports', rule: 'Every shared internal is used by something', why: '_shared/ holds the parts several components are built from, and they are invisible twice over: no spec, so no spec-driven check sees them; and no consumer outside the library, so nothing complains when the last importer drops them. YearPicker and MonthPicker sat there with zero references \u2014 two components and two stylesheets every reader had to consider and no build ever used. Dead code in a shared directory costs more than elsewhere, because _shared/ is where someone looks for the piece they should reuse.', enforcement: C('check', 'shared-orphans') },
  { id: 'exports-7', group: 'exports', rule: 'Every component hook is called by something, and its spec says it exists', why: 'A hook beside a component that nothing calls is a second implementation of behaviour the component already has \u2014 and it is the copy that stops being maintained. useDatePicker was 444 lines of public API called by nothing: not DatePicker, which keeps its own state, not the docs, not a test. A bug fixed in the component was still in the hook hours later, because fixing one place cannot fix the other. A caller can be the component, a sibling hook, a test, or a docs sample \u2014 useTableSelection is called by no component because the selection belongs to the call site, and the sample is what proves it works.', requires: ['factory'], enforcement: C('check', 'component-hooks') },
  ...
  { id: 'behavior-4', group: 'behavior', rule: "The caller's accessible name wins; the component only defaults it", why: "Every component spreads the caller's props onto its element, and JSX compiles that to an object literal — so an attribute set AFTER the spread replaces what they passed, and a conditional one resolving to undefined DELETES it, leaving the element anonymous. Both are silent. A component can only supply a default name: it knows the control is a close button, not whether this one closes a dialog or a filter panel, so aria-label, aria-labelledby, aria-describedby, aria-valuetext, title and alt must precede the spread or read the caller's value back. Structural props are the opposite case and stay after it — role, type and aria-expanded ARE the component, and a caller overriding one breaks the contract it guarantees. DOM handlers are the third case, where neither wins and both must run: compose with composeHandlers(attrs.onClick, ours), which runs the caller's first and skips the component's if they called preventDefault — so a caller who wants the click without the sort has somewhere to stand.", enforcement: C('check', 'prop-precedence') },
  ...
  { id: 'apiSurface-3', group: 'apiSurface', rule: 'A compound part that must contain another one, does', why: 'A compound API is a set of parts the caller assembles, and most assemblies are a matter of taste \u2014 some are not. Select.Content needs a Select.Viewport, because Viewport holds the container the reveal animation targets and takes its ref from. The failure is silent, which is the whole point: omit it and the select still renders, opens, selects and closes, while the reveal never runs. CalendarNav omitted it for the entire life of the Radix Select rebuild and nothing said a word \u2014 not tsc, not 2313 tests, not fifty checks; a person noticed the months did not fade in. Declared as data on the sub-component, so the check reads the requirement rather than hardcoding one library\u2019s shape, and scanned across docs too, since a sample that omits it teaches every reader to omit it.', enforcement: C('check', 'required-children') },

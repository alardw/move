...
const RULES: RuleDef[] = [
  ...
  { id: 'source-4', group: 'source', rule: 'Default values in the defaults object', why: 'The factory reads defaults from one place; inline defaults in destructuring bypass it and drift from the spec.', enforcement: C('gap') },
  ...
  { id: 'source-6', group: 'source', rule: 'cx() for every slotted className', why: 'cx() merges the slot class with the consumer’s className; a raw class drops the consumer’s styling for that part.', enforcement: C('gap') },
  ...
  { id: 'source-9', group: 'source', rule: 'ref forwarded to root', why: 'Focus, measurement, and portals need a handle on the real DOM node; a dropped ref breaks all three.', enforcement: C('gap') },

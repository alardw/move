...
const hueTrackStyle = {
  '--move-range-track-bg': HUE_GRADIENT,
  ...
  return (
    ...
    >
      ...
      <Text size="sm" color="muted">
        A supporting detail line in the secondary text tier.
      </Text>
      ...
      <Text size="xs" color="subtle">
        Subtle footnote — timestamps, hints, metadata.
      </Text>
    </Stack>
  );
}
...
export function ThemeBuilderPage() {
  ...
  React.useEffect(() => {
    [headingFont, bodyFont].forEach((label) =>
      loadGoogleFont(FONTS.find((f) => f.label === label)?.google ?? null),
    );
  ...
  const paletteSwatches = React.useMemo(
    () =>
      PALETTE.map((p) => ({
        name: p.name,
        color: harmonize(p, paletteHarmonize ? accentChroma : 0.16),
      })),
    ...
    <Stack direction="row" gap="xl" align="stretch" id="theme">
      <Stack gap="xl" flex={1}>
        ...
        <Section id="import" title="Import your theme">
          <Text color="muted">
            Already have a design system — from Tailwind, Figma, or Material? Bringing it in is a
            short list: its accent and gray, its corner radius, and its fonts. Read those off the
            source, enter them here, and Move reproduces the rest — every surface, border, and state
            — with the contrast kept honest. Or hand the whole job to an assistant: the{' '}
            <Link asChild>
        ...
        <Section
          ...
          lede="Two colors in — a full, accessible theme out. The preview is the proof: it's the real theme, shown in both modes."
        >
          ...
          <Stack direction="row" gap="lg" collapseBelow="880px" align="stretch">
            ...
            >
              <Stack gap="sm">
                ...
                <Stack direction="row" gap="sm" align="center">
                  <div
                    style={{
                      ...
                      borderRadius: 8,
                      flex: 'none',
                      background: oklchHex(0.57, accentChroma, accentHue),
                      border: '1px solid var(--move-border-base)',
                    }}
                  />
                  <div style={{ flex: 1 }}>
                ...
                <Text size="sm" color="subtle">
                  Buttons, links, highlights, and focus outlines.
                </Text>
              </Stack>
              ...
              <Stack gap="sm">
                <Text weight="medium">
                  Accent saturation —{' '}
                  {accentChroma === 0 ? 'greyscale' : `${Math.round((accentChroma / 0.26) * 100)}%`}
                </Text>
                <InputRange
                  ...
                  style={
                    {
                      '--move-range-track-bg': `linear-gradient(90deg, ${oklchHex(0.57, 0, accentHue)}, ${oklchHex(0.57, 0.26, accentHue)})`,
                      '--move-range-range-bg': 'transparent',
                    } as React.CSSProperties
                  }
                />
                <Text size="sm" color="subtle">
                  From a fully greyscale accent up to a vivid one. Legibility holds at every
                  setting.
                </Text>
              </Stack>
                ...
                <Stack direction="row" gap="sm" align="center">
                  <div
                    style={{
                      ...
                      borderRadius: 8,
                      flex: 'none',
                      background: oklchHex(0.6, Math.max(neutralChroma * 1.4, 0.004), neutralHue),
                      border: '1px solid var(--move-border-base)',
                    }}
                  />
                  <div style={{ flex: 1 }}>
                    <InputRange
                      aria-label="Background tint hue"
                      min={0}
                      max={360}
                      step={1}
                      value={neutralHue}
                      onValueChange={(v) => setNeutralHue(v[0])}
                      style={hueTrackStyle}
                    />
                  </div>
                </Stack>
                <Text size="sm" color="subtle">
                  Lean your grays warm or cool. Backgrounds are never pure white or black, so the
                  mood carries through.
                </Text>
              </Stack>

              <Stack gap="sm">
                <Text weight="medium">
                  Tint strength — {Math.round((neutralChroma / 0.045) * 100)}%
                </Text>
                <InputRange
                  ...
                  step={0.001}
                  value={neutralChroma}
                  onValueChange={(v) => setNeutralChroma(v[0])}
                  style={chromaTrackStyle}
                />
                <Text size="sm" color="subtle">
                  How strongly that tint shows — from plain gray to clearly colored.
                </Text>
              </Stack>

              <Stack gap="sm">
                <Text weight="medium">
                  Corner radius — {radius === 0 ? 'sharp' : `${radius.toFixed(1)}×`}
                </Text>
                <InputRange
                ...
                />
                <Text size="sm" color="subtle">
                  Sharp to rounded — scales every corner at once; pills stay pills.
                </Text>
              </Stack>
              ...
              <Stack gap="sm">
                ...
                <Text size="sm" color="subtle">
                  Previewed from Google Fonts; ship self-hosted via Fontsource in production.
                </Text>
              </Stack>
              ...
              style={
                {
                  ...
                  minWidth: 0,
                  ...both.radius,
                  '--move-font': fontStack(bodyFont),
                  '--move-font-body': fontStack(bodyFont),
                  '--move-font-heading': fontStack(headingFont),
                } as React.CSSProperties
              }
            >
          ...
          <div style={{ marginTop: 16 }}>
            {violations === 0 ? (
              <Alert variant="success" title="Contrast meets WCAG 2.2 AA">
                Every text style, link, button label, and focus ring is legible in both light and
                dark
                {nudged > 0
                  ? ` — ${nudged} color${nudged > 1 ? 's were' : ' was'} adjusted automatically to keep it that way`
                  : ''}
                . Contrast is one part of AA —{' '}
              ...
              <Alert variant="danger" title="Contrast issue">
                {violations} pairing{violations > 1 ? 's fall' : ' falls'} below the WCAG 2.2 AA
                contrast minimum. Ease the tint or shift a hue.
              </Alert>
            )}
          </div>
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {showDetails && (
              <Stack gap="none" style={{ marginTop: 8 }}>
                ...
                >
                  <Text size="sm" color="subtle" weight="medium">
                    Contrast
                  </Text>
                  <Text size="sm" color="subtle" weight="medium">
                    Light
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        alignItems: 'center',
                        paddingBlock: 6,
                        borderTop: '1px solid var(--move-border-base)',
                      }}
                    >
        ...
        <Section
          ...
          lede="One call sets the whole theme. An AI assistant can write it for you too."
        >
          <Stack gap="md">
            ...
            {fontCss && (
              <Stack gap="xs">
                <Text size="sm" color="muted">
                  ...
                  <Link asChild>
                    <RouterLink to="/customize/typography">
                      Loading, scoping, sizes &amp; weights →
                    </RouterLink>
                  </Link>
                </Text>

...
- Unify the i18n label API (audit 2026-06-25). Three patterns coexist today:
  (a) `labels` object — Alert, Calendar, DatePicker, Password;
  (b) flat `*Label?:` props — AudioPlayer, VideoPlayer, Autocomplete, CalendarView, ColorInput, ColorPicker, FileUpload, NumberInput, PinInput, Popover, ProgressBar, RichTextEditor, Toast;
  (c) hardcoded `aria-label="…"` literals (NOT overridable — i18n gaps) — Breadcrumb ("Breadcrumb"), Pagination ("Go to next/previous page", "Pagination"), Sidebar ("Close sidebar"), Loader ("Loading"), TimeField ("period"), ColorPicker channels ("Alpha"/"Color format"/"Hex color value"), FileUpload ("Upload complete"), TableOfContents ("On this page").
  Target: every user-facing string on a `labels` object (`labels?: Partial<XLabels>`) with English defaults; zero hardcoded aria-label literals. Migrate (b)→(a), add `labels` to (c). Then update each spec's `labels` field + regenerate, expand the Internationalization docs table to all components, and tighten validate **E1** to require the `labels` object AND flag hardcoded `aria-label` string literals (so it can't recur). WCAG note: most icon buttons already have accessible names — this is about consistency + localizability, not missing names.

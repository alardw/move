...
- **Prop precedence, behaviour tranche: 55 sites swallow the caller's handler.** `check:prop-precedence --handlers` reproduces the list. A component setting `onClick` after `{...attrs}` replaces what the caller passed; when it is conditional it is worse — `Table.tsx:602` renders `onClick={sortable ? handleClick : undefined}`, so a non-sortable header DELETES a caller's handler. Unlike naming (caller wins) and structural (component wins), handlers have no winner: BOTH must run. The correct shape already exists in the library — `Carousel.tsx:481` does `const { onClick: userOnClick, ...restAttrs } = attrs` and calls both, as does Button for its six mouse/key handlers. Left out of the naming fix deliberately: composing 55 handlers changes runtime behaviour in ~20 components (handlers that never fired start firing, which is the point but is still a behaviour change), and it does not belong in the same commit as a rule about names. Enforce the `handler` kind in `prop-precedence.mjs` by deleting the `HANDLERS` flag once the sites are fixed.

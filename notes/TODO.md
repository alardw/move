...
- **Nothing enforces that a new guest scopes its state.** Same date, same commit. `src/engine/ScopedSlot.test.tsx` pins the six that exist (Tooltip, Dropdown, Popover, Dialog, Drawer, Collapsible) by composing each onto a `ToggleGroup.Item` and asserting the host keeps `data-state`. A seventh `asChild` trigger added later passes every check while quietly taking its host's state. The rule is structural — a component offering `asChild` must not emit a bare `data-state` on that path — so it is checkable over the same source, but it needs a real check file plus its `@enforces` rule in `conformance-spec.ts` and the docs registry, which the `rule-coverage` bijection requires.

...
- **i18n hole: computed user-facing strings evade both guards.** The label API migration (DONE 2026-06-26, below) removed hardcoded `aria-label` literals and JSX prose, but neither guard sees a string ASSEMBLED and passed through a variable: conformance E1 refuses a *literal* `aria-label` value, and `check:i18n-literals` scans *JSX text children*. A template literal built in a helper and handed to `aria-label={name}` passes both. Found via Chart, whose accessible name was built from `'rises'`/`'peaking at'`/`'points.'` (fixed 2026-08-27 — it now goes through `labels.summary`, a function taking derived facts, since word order and verb agreement move between languages).

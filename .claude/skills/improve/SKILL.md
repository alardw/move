...
| File | Purpose |
|------|---------|
| `_reference/spec-type.ts` | `ComponentSpec` type |
| `_reference/categories.ts` | Valid categories |
| `_reference/animation-map.ts` | Class → animation wiring |
| `_reference/a11y-contract.ts` | Class → ARIA requirements |
| `_reference/default-conventions.ts` | Token/variant defaults |
...
| Change animation | `animations`, `animationImports` |
...
- **Token values**: validate against `_reference/tokens-semantic.ts` (same rule as `/spec`)
...
### Changed
- animations[0].configType: 'LifecycleAnimate' → 'LifecycleAnimate & InteractionAnimate'
...
4. **Token validation** — all new token values must reference `var(--move-*)` from `_reference/tokens-semantic.ts`

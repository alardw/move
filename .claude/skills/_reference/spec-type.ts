...
//     - defaultProfile: named profile key from _reference/animation-map.ts
...
export const SPEC_SCHEMA_VERSION = 6 as const;
...
export interface AnimationBinding {
  ...
  hook: string;
  /** Animation config type as trigger intersection (e.g. 'LifecycleAnimate & InteractionAnimate') */
  configType: string;
  /** Legacy default animation preset key from defaultAnimations */
  defaultPreset?: string;
  /** Named profile from _reference/animation-map.ts (preferred over defaultPreset) */
  defaultProfile?: string;
  /**
   * Explicit default config literal (TypeScript object-as-string).
   * When present, generation MUST use this exact config and not normalize
   * to a class preset. Example:
...
export interface DefaultReview {
  ...
  decisionSource: 'user-confirmed' | 'accept-all';

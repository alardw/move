...
4. `_reference/radix-testing-patterns.ts` — if component wraps a Radix primitive (check for `@radix-ui` imports in source)
...
| `interactive` | + click handler, disabled state, keyboard (Space/Enter), animation prop |
...
10. **spec.testing.animation entries** become animation prop tests
...
When generating tests for components that wrap Radix UI primitives, read `_reference/radix-testing-patterns.ts` for known jsdom quirks and required patterns:

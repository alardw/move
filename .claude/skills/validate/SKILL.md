...
| A13 | Animation defaults match spec precedence | defaultConfig/defaultProfile preserved; no generic downgrade |
...
| B7 | All `var(--move-*)` references resolve to real tokens — cross-check against `_reference/tokens-primitive.ts` and `_reference/tokens-semantic.ts` |
...
Also validate that:
- All spec tokens reference real tokens from `_reference/tokens-primitive.ts` and `_reference/tokens-semantic.ts`
...
- Spec animation defaults match generated source (`defaultConfig`/`defaultProfile` precedence)

 ...
 */
function toTriggerInteractiveAnimation(
  animation: Animation
): Animation {
  ...
  for (const [key, value] of Object.entries(animation)) {
    ...
    // Simple value or other property
    else {
      result[key] = value;
    }
  }
...
>(({ className, children, icon, animate: animateProp, ...props }, ref) => {
  ...
  const handleMouseEnter = () => {
    ...
    const processed = toTriggerInteractiveAnimation(config.hover);
  ...
  const handleMouseLeave = () => {
    ...
    const neutralScale = getTriggerResetScale(config.hover);

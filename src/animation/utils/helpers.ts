...
import { springs, isSpring, DEFAULT_DURATION, type AnimationPreset } from '../easings';
...
export function toAnimeParams(
  ...
  for (const [key, value] of Object.entries(properties)) {
    ...
    if (isPerPropertyEasing(value)) {
      hasPerPropertyEasings = true;
      // Per-property easing: { value: [0, 1], easing: 'outQuart' }
      const propEasing = value.easing ?? animationEasing ?? fallbackEasing;
      const resolvedEase = resolveEasing(propEasing);

      if (Array.isArray(value.value)) {
        // Array [from, to] - use from/to syntax for anime.js v4
        const [fromVal, toVal] = value.value as [unknown, unknown];
        result[key] = {
          from: fromVal,
          to: toVal,
          ease: resolvedEase,
          ...(!isSpringEasing(propEasing) && { duration: duration ?? DEFAULT_DURATION }),
        };
      } else {

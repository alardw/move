...
| B7 | All `var(--move-*)` references resolve to real tokens — cross-check against `references/tokens-primitive.ts` and `references/tokens-semantic.ts` |
...
Also validate that:
- All spec tokens reference real tokens from `references/tokens-primitive.ts` and `references/tokens-semantic.ts`

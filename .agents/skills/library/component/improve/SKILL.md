...
| File | Purpose |
|------|---------|
| `references/spec-type.ts` | `ComponentSpec` type |
| `references/categories.ts` | Valid categories |
| `references/animation-map.ts` | Class → animation wiring |
| `references/a11y-contract.ts` | Class → ARIA requirements |
| `references/default-conventions.ts` | Token/variant defaults |
| `references/engine-api.ts` | Available imports |
...
- **Token values**: validate against `references/tokens-semantic.ts` (same rule as `/spec`)
...
4. **Token validation** — all new token values must reference `var(--move-*)` from `references/tokens-semantic.ts`

...
| File | Purpose |
|------|---------|
| `references/factory-contract.md` | `withMoveComponent` signature, setup pattern, render pattern |
| `references/css-contract.md` | Token placement, data-attribute selectors, animation prohibition |
| `references/engine-api.ts` | Available imports (engine, animation, infrastructure) |
| `references/animation-map.ts` | Animation patterns and presets |
| `references/infrastructure.ts` | Infrastructure imports (useResolvedIcon, etc.) — read if spec uses built-in icons |
...
9. **Token validation (mandatory)** — before emitting any `var(--move-*)` reference in CSS, verify the token exists:
   - Read `references/tokens-semantic.ts` for semantic tokens (`--move-spacing-*`, `--move-rounded-*`, etc.)
   - Read `references/tokens-primitive.ts` for primitive tokens (`--move-space-*`, `--move-radius-*`, etc.)
...
8. **Use infrastructure when needed** — if the spec has `componentDeps`, read `references/infrastructure.ts` and use the appropriate imports in the generated source
...
18. **Barrel-only imports** — ALWAYS import from `'../../../engine'` and `'../../../animation'`, NEVER from sub-paths like `'../../../engine/types'`, `'../../../animation/hooks'`, `'../../../animation/easings'`, etc. Both modules re-export everything from their barrel `index.ts`. Read `references/engine-api.ts` IMPORT_PATTERNS for canonical import lines.
